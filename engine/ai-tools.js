...
(function () {
  ...
  window.AI_TOOLS['invoice-extractor'] = {
    ...
    inputs: [{ key: 'doc', label: 'Invoice or receipt', type: 'text+file', accept: '.pdf,.txt,.md', rows: 12, placeholder: 'Paste the invoice text, or choose the PDF above. Scanned PDFs have no text layer and cannot be read here. For a scan or a photo use the Scanned Invoice & Receipt Reader.' }],
    ...
    tips: ['A typed or exported PDF has a text layer and reads cleanly. A scanned image does not — the page tells you, nothing is sent, and the Scanned Invoice & Receipt Reader reads it as a picture instead.', 'Check the totals against the paper before you post them. The model reads well, but it is reading, not calculating; the line items are there so you can add them up.', 'Download the .xlsx and the line items come as a proper table under the header fields, ready for a purchase register or a Tally import.', 'Multi-page invoices work; the first 40 pages are read. Statements with dozens of invoices are better fed one invoice at a time.'],
    faq: [{ q: 'What is sent, and where?', a: 'The text of the invoice — what you pasted, or what the page read out of the PDF — is sent to the model through our gateway to be structured. The PDF file never leaves your device, and we do not store the text or the answer; only the fact that a call was made is counted against your monthly allowance.' }, { q: 'How accurate is it?', a: 'On typed invoices, very: vendor, numbers, dates and line items come back correctly in the great majority of cases. Where a value is ambiguous the model is told to return null rather than guess. Treat it as a fast first draft of data entry that you check, not as an auditor.' }, { q: 'Does it do OCR on scanned invoices?', a: 'Not this tool: it reads the text layer and sends no picture. The Scanned Invoice & Receipt Reader sends the picture itself to the model and reads it from the pixels — use that for scans and photos, and this one for typed PDFs, which is cheaper on tokens and keeps the file on your device.' }]
  };
